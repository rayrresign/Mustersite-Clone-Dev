...
// Content Post Slider - Carousel auf Modalbox


function resign_post_slider($slider_id, $post_slider_customposttype, $post_slider_columns, $post_slider_mode, $post_slider_interval, $post_slider_width ) {
		...
		if ($the_query->have_posts()): 
			...
			$gallery_slider_start = '<div id="'. $slider_id .'" class="carousel slide content-slider'.$fade.'" data-ride="carousel"'.$post_slider_interval.'><div class="carousel-inner" role="listbox">';
			$gallery_slider_end = '</div>';
			
			$gallery_slider_controls = '<div class="controls">
			<a class="left" href="#'. $slider_id .'" role="button" data-slide="prev">
        	<div class="arrow-left" aria-hidden="true"></div>
        	<span class="sr-only">Previous</span>
      		</a>
			  <a class="right" href="#'. $slider_id .'" role="button" data-slide="next">
		...
		while ($the_query->have_posts()) : $the_query->the_post();
				...
				if ($i % $post_slider_columns == 0) {  // Wenn durch Anzahl Spalten teilbar
					...
					$indicator .= '<li data-target="#'. $slider_id .'" data-slide-to="'.$j.'"'.$indicator_active.'></li>';
				...
				if ($post_count == $i) { // Wenn letzter Slide
					...
					$indicator .= '<li data-target="#'. $slider_id .'" data-slide-to="'.$j.'"></li>';
					$j = $j +1;
					}
				}
				...
				$slides_output .= '<div class="post-carousel-item contentBox">';
				$slides_output .= '<a href="#" class="load-content" data-target="contentModalBox" data-id="'. get_the_ID() .'" data-thumb="true">';
		...
		endwhile; 
			...
			return '<div role="alert" class="alert alert-info">...</div>';
		endif;
