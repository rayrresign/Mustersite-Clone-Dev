...
function resign_testimonials_slider($id, $post_slider_customposttype, $post_slider_columns, $post_slider_mode, $post_slider_interval, $post_slider_width)
{
    ...
    if ($the_query->have_posts()):
        ...
        while ($the_query->have_posts()) : $the_query->the_post();
            ...
            if ($post_slider_width == 'full') {
                $thumbnail_url = wp_get_attachment_image_src($thumbnail_id, 'res-quadrat-thumbnail', true);
            } else {
                $thumbnail_url = wp_get_attachment_image_src($thumbnail_id, 'res-quadrat-thumbnail', true);
            }
            ...
            if ($post_count == $i) { // Wenn letzter Slide
				...
				$slides_output .= '<div class="post-carousel-item text-center testimonialsBox">';
				$slides_output .= '<div class="postTxt"><h3>'.get_the_title().'</h3> <p>'.get_the_excerpt().'</p></div>';
				$slides_output .= '<div class="postName"><p>Michael Maier aus Winterthur </p></div>';
				$slides_output .= '<img class="img-responsive center-block testimonial-image" src="' . $thumbnail_url[0] . '" alt="" />';
				$slides_output .= '</div>';
				$slides_output .= '</div>';
